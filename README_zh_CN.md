...
> 当前版本: v1.2.0
>
> 新增：MCP访问权限控制改版，可为单独文档设定读/写/破坏性修改权限（需要在文档树右键菜单设定）；
>
...
- 大部分工具提供排除文档功能；
- 有一定的输入参数检查和限制，并不是直接将思源笔记的API包装为工具；
- 在电脑客户端安装插件即可使用；不支持Docker和移动端；
- 在服务端提供审阅功能，可点击操作对象ID跳转到对应文件或块；
...
|          |     |                         | 获取文档的子文档列表                |   ✅   | —                                                                                               |
|          |     |                         | 获取子块列表                        |   ✅   | —                                                                                               |
|          |     |                         | 向量检索客户端插件-查询             |    ✅    | 使用此功能需要下载并正确配置[syplugin-vectorIndexClient](https://github.com/OpaqueGlass/syplugin-vectorIndexClient)插件<br />                                     |
|          |     |                         | 向量检索客户端插件-获取可用检索服务 |    ❌    | —                                                                                               |
|          |     |                         | 通过 ID 获取反向链接                |    ✅    | —                                                                                               |
|          | 写  |                         | 向日记追加内容                      |   ✅   | 跟随笔记本权限设定                                                                               |
...
|          |     |                         | 通过块 ID 创建闪卡                  |    ✅    | 检查是否对该块具有读权限；如要禁止创建闪卡，请在客户端关闭此工具                                 |
|          |     | 是/否                   | 通过块 ID 删除闪卡                  |    ❌    | —                                                                                               |
| 模板     |     |                         |                                     |          |                                                                                                  |
|          | 读  |                         | 检索已有模板                        |    ❌    | —                                                                                               |
|          |     |                         | 模板文件原始内容                    |    ❌    | —                                                                                               |
